-- Data Preparation: Cleaning, Transforming, and Structuring for use
...
cleaned_orders AS (
  SELECT 
    *,
    CASE WHEN LOWER(cleaned_product_info.product_name) LIKE '%samsung%' THEN 'Samsung'
	WHEN LOWER(cleaned_product_info.product_name) LIKE '%lenovo%' THEN 'Lenovo'
	WHEN LOWER(cleaned_product_info.product_name) LIKE '%apple%' THEN 'Apple'
    WHEN LOWER(cleaned_product_info.product_name) LIKE '%bose%' THEN 'Bose'
    ELSE 'Unknown' 
	END AS brand_name,
    CASE WHEN LOWER(cleaned_product_info.product_name) LIKE '%iphone%' THEN 'Cell Phone'
    WHEN LOWER(cleaned_product_info.product_name) LIKE '%monitor%' THEN 'Monitor'
	WHEN LOWER(cleaned_product_info.product_name) LIKE '%headphones%' THEN 'Headphones'
    WHEN LOWER(cleaned_product_info.product_name) LIKE '%laptop%' THEN 'Laptop'
    ELSE 'Accessories' 
	END AS product_category,
    ROW_NUMBER() OVER (PARTITION BY cleaned_product_info.customer_id ORDER BY cleaned_product.purchase_date) AS customer_order_number,
    EXTRACT(YEAR FROM cleaned_product_info.purchase_date) AS year,
    CASE WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 1 THEN 'January'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 2 THEN 'February'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 3 THEN 'March'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 4 THEN 'April'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 5 THEN 'May'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 6 THEN 'June'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 7 THEN 'July'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 8 THEN 'August'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 9 THEN 'September'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 10 THEN 'October'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 11 THEN 'November'
    WHEN EXTRACT(MONTH FROM cleaned_product_info.purchase_date) = 12 THEN 'December'
    ELSE NULL END AS month
    FROM cleaned_product_info
),

-- Cleaning Customer Information and Handling Nulss

cleaned_customers AS (
  SELECT
    customers.id AS customer_id,
	ISNULL(customers.marketing_channel, 'unknown'),
	ISNULL(customers.account_creation_method, 'unknown'),
...
-- Standardizing Region and Country
...
cleaned_order_status AS (
  ...
  FROM TechHive.order_status
  LEFT JOIN TechHive.orders
   ON order_status.order_id = orders.id
  LEFT JOIN TechHive.customers
